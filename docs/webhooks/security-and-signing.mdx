...
Webhooks include an HMAC-SHA256 signature in the `1sub-signature` header:
...
Parse the `1sub-signature` header to get timestamp and signature:
...
func webhookHandler(w http.ResponseWriter, r *http.Request) {
    signature := r.Header.Get("1sub-signature")
