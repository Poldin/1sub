...
### Primary Linking Flow: Email-Based (SaaS Tools)

For web applications where users have verified emails:

<Steps>
<Step title="User subscribes on 1Sub">
  User completes subscription on the 1Sub platform with their email.
</Step>

<Step title="User logs into your tool">
  User logs into your tool with the same email they used on 1Sub.
</Step>

<Step title="Hash email and verify subscription">
  Your tool hashes the user's email and calls the verify endpoint:
...
// Verify subscription
const subscription = await verifySubscription({ emailSha256: emailHash });
```
</Step>

<Step title="Extract and cache oneSubUserId">
  Extract the `oneSubUserId` from the response and cache it:

```typescript
const oneSubUserId = subscription.oneSubUserId;

// Store for future lookups (10-20x faster)
await db.users.update(localUserId, { oneSubUserId });
```
</Step>
...
VALUES ('user_123', 'uuid-from-verify')
...
<Tip>
After the first email lookup, always use the cached `oneSubUserId` for subsequent verifications - it's 10-20x faster than email lookups.
</Tip>

### Alternative Linking Flow: Link Codes
...
<Step title="Store the link">
  Save the mapping in your database (same as email-based flow).
</Step>
</Steps>
