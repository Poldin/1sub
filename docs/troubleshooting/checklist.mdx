...
- [ ] Email hashing implemented correctly (SHA256 of lowercase, trimmed email)
- [ ] Link code exchange endpoint is implemented
- [ ] Link code validation (6-10 alphanumeric characters)
- [ ] Expired link codes (>10 min) are rejected with clear error
- [ ] Invalid link codes return helpful error messages
- [ ] `oneSubUserId` is cached after first lookup for performance
- [ ] User cannot link multiple 1Sub accounts to same tool account
- [ ] Link conflicts are handled gracefully
</Accordion>
...
<AccordionGroup>
<Accordion title="✅ User Account Linking">
- [ ] Email-based linking is implemented (for SaaS tools) OR
- [ ] Link code exchange is implemented (for CLI/desktop apps)
- [ ] `oneSubUserId` is extracted from verify response
...
- [ ] First-time linking uses emailSha256, subsequent uses oneSubUserId
</Accordion>
...
- [ ] `oneSubUserId` lookups are preferred over email lookups (10-20x faster)
- [ ] Cache implementation is production-ready (Redis recommended)
</Accordion>
...
- [ ] Email hashing is tested (SHA256 of lowercase, trimmed)
- [ ] Link code validation is tested
...
- [ ] User linking logic is tested (both email and link code)
</Accordion>
...
- [ ] Test link code expiration (>10 minutes)
- [ ] Test email-based linking with matching/non-matching emails
- [ ] Test rate limiting
