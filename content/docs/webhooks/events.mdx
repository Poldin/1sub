...
<Note>
This event uses a slightly different payload structure than other events. It uses `event` instead of `type`, and `timestamp` (ISO 8601 string) instead of `created` (Unix timestamp).
</Note>
...
When the credits were consumed
</ResponseField>
...
<Expandable title="Data Fields">
  ...
  <ResponseField name="balance_remaining" type="number" required>
  User's remaining credit balance after this transaction
  </ResponseField>

  <ResponseField name="transaction_id" type="string (UUID)" required>
...
case 'credits.consumed':
  const { user_id, amount, balance_remaining, transaction_id } = event.data;

  // Log credit consumption for analytics
  await logCreditUsage(user_id, {
    amount,
  ...
  if (balance_remaining < 50) {
    await sendLowBalanceWarning(user_id, balance_remaining);
  }

  // Update usage metrics
  await updateUsageMetrics(user_id, amount);

  // Track for billing/reporting
  await recordCreditConsumption({
    userId: user_id,
    creditsUsed: amount,
    remainingBalance: balance_remaining,
    transactionId: transaction_id
  });
