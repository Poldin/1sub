...
1Sub sends webhooks for user registration, subscription lifecycle, purchases, credit status, entitlements, credit consumption, and system events:
...
- Send welcome email
- Prepare user account before subscription
...
<Tip>
Set up co-branded registration in your Vendor Dashboard → API & Webhooks → "Co-branded Registration" section.
</Tip>
</Accordion>
</AccordionGroup>
...
Event type: `user.registered`, `subscription.activated`, `subscription.updated`, `subscription.canceled`, `purchase.completed`, `user.credit_low`, `user.credit_depleted`, `tool.status_changed`, `entitlement.granted`, `entitlement.revoked`, `entitlement.changed`, `verify.required`, or `credits.consumed` (note: `credits.consumed` uses `event` field instead)
</ResponseField>
