...
curl -X POST 'http://localhost:3000/webhooks/1sub' \
  -H 'X-1Sub-Signature: t=1700000000,v1=your-computed-signature' \
  ...
  describe('Signature Verification', () => {
    it('accepts valid signature', async () => {
      ...
      const response = await request(app)
        .post('/webhooks/1sub')
        .set('X-1Sub-Signature', signature)
    ...
    it('rejects invalid signature', async () => {
      ...
      const response = await request(app)
        .post('/webhooks/1sub')
        .set('X-1Sub-Signature', 't=123,v1=invalid')
        .set('Content-Type', 'application/json')
        .send(payload);
    ...
    it('rejects old timestamp', async () => {
      ...
      const response = await request(app)
        .post('/webhooks/1sub')
        .set('X-1Sub-Signature', `t=${oldTimestamp},v1=${sig}`)
        .set('Content-Type', 'application/json')
        .send(payload);
