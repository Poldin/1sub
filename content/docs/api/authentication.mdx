...
response = requests.post(
    'https://1sub.io/api/v1/verify',
    json={'verificationToken': 'vt_xxxxxxxxxxxxxxxxxxxxxxxx'},
    headers={
...
# Expected response if auth works but token is invalid:
# {"valid": false, "error": "INVALID_TOKEN", "reason": "Token verification failed"}

# If auth fails, you'll get:
# {"valid": false, "error": "UNAUTHORIZED", "reason": "Invalid API key"}
```
