...
| 401 | Unauthorized | Invalid/missing API key or token expired | Verify API key or reauthenticate |
| 402 | Payment Required | No active subscription or insufficient credits | User needs to subscribe or add credits |
| 403 | Forbidden | Access revoked or tool inactive | Terminate user session immediately |
...
When a user's access is revoked (subscription cancelled, payment failed, fraud, etc.), 1Sub enforces this across all endpoints:

### Revocation Flow

1. **Webhook Notification** (immediate)
   - You receive an `entitlement.revoked` webhook
   - Invalidate any cached user data
   - Update your database to mark access as ended

2. **API Enforcement** (on next request)
   - Next `/verify` call returns: `{ valid: false, action: "terminate_session" }`
   - `/credits/consume` returns: `403 Forbidden` with revocation reason
   - User can no longer access your tool

### Implementation Guide

```typescript
// 1. Handle the webhook (proactive notification)
app.post('/webhooks/1sub', (req, res) => {
  const event = req.body;

  if (event.type === 'entitlement.revoked') {
    const { onesubUserId, toolId, reason } = event.data;

    // Invalidate cached data
    cache.delete(`user:${onesubUserId}`);

    // Mark in database
    await db.users.update(onesubUserId, { accessRevoked: true });

    console.log(`Access revoked for ${onesubUserId}: ${reason}`);
  }

  res.status(200).send('OK');
});

// 2. Check on every request (enforcement guarantee)
app.use(async (req, res, next) => {
  ...
  if (!result.valid) {
    ...
    req.session.destroy();
    return res.status(403).render('access-ended', {
      reason: result.reason,
      action: result.action // 'terminate_session' or 'reauthenticate'
    });
  }

  // Update session with new token (token rolling)
...
<Warning>
**Critical:** When you receive `valid: false` with `action: "terminate_session"`, you MUST:
1. Terminate the user's session immediately
2. Clear any cached data
3. Show a clear "Your access has ended" message
4. Do NOT allow further access to paid features
</Warning>

<Tip>
**Performance:** Cache the verification result for up to `cacheUntil` seconds, but always check the webhook first for immediate revocation notifications.
</Tip>
...
|--------|-------------|-------------|
| `subscription_cancelled` | User cancelled their subscription | Resubscribe on 1Sub |
| `payment_failed` | Payment method failed | Update payment method |
| `bundle_changed` | Tool removed from user's bundle | Add tool back to bundle |
| `manual` | Admin manually revoked access | Contact support |
| `fraud` | Detected fraudulent activity | Contact support |
| `tool_deactivated` | Tool is no longer active | Tool unavailable |
