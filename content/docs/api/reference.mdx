...
**keys** (array, required): Array of JWK (JSON Web Key) objects

Each JWK contains:
- **kty** (string): Key type, always `"RSA"`
- **kid** (string): Key ID, used to identify which key signed a JWT
- **use** (string): Key usage, always `"sig"` (signature)
