...
```json
{
  ...
  "message": "Missing required parameter: verificationToken"
}
```
...
User has no active subscription or insufficient credits.

**Common causes:**
- User's subscription is cancelled or expired
...
if (error.response?.status === 402) {
  if (error.response.data.error === 'Payment required') {
    ...
    showCreditPurchasePrompt(error.response.data);
  } else {
...
**Common causes:**
- **Access revoked** (most critical - user's subscription cancelled, fraud, etc.)
...
if (error.response?.status === 403) {
  ...
  if (data.action === 'terminate_session') {
    // CRITICAL: Access has been revoked
    // Immediately terminate user session
    req.session.destroy();
    return res.render('access-ended', {
      reason: data.reason,
      message: 'Your access to this tool has ended'
    });
  }
...
<Warning>
**Critical:** When you receive a 403 with `action: "terminate_session"`, you MUST immediately:
1. Terminate the user's session
2. Clear all cached data
3. Show a clear "access ended" message
4. Block further access to paid features
</Warning>
</Accordion>
...
function handleAPIError(error: any) {
  ...
  switch (status) {
    ...
    case 401:
      ...
      break;
    case 402:
      console.warn('Payment required:', data.message);
      break;
    case 403:
      if (data.action === 'terminate_session') {
        console.error('Access revoked:', data.reason);
        terminateSession();
      } else {
        console.error('Permission denied:', data.message);
      }
...
const ERROR_MESSAGES = {
  'Payment required': 'You don\'t have enough credits. Purchase more to continue.',
  'NO_SUBSCRIPTION': 'Please subscribe to this tool on 1Sub first.',
  'ACCESS_REVOKED': 'Your access to this tool has ended.',
  'Forbidden': 'Your access has been revoked.',
  'Invalid API key': 'Configuration error. Please contact support.',
  'Rate limit exceeded': 'Too many requests. Please wait a moment and try again.',
  'Invalid or expired code': 'This code has expired. Please click "Launch Tool" again.',
};
...
async function safeConsumeCredits(params: ConsumeCreditsParams) {
  try {
    ...
    if (error.response?.status === 402) {
      ...
      if (data.error === 'Payment required') {
        return {
          ...
          message: `You need ${data.shortfall} more credits.`,
    ...
    if (error.response?.status === 403) {
      // Access revoked - terminate session immediately
      return {
        success: false,
