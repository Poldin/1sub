/**
 * Redis Cache Layer for Entitlements
 *
...
export async function invalidateAllUserEntitlements(userId: string): Promise<void> {
  ...
  try {
    ...
    if (redis) {
      // Note: Pattern-based deletion requires SCAN which isn't available in REST API
      // For Upstash, this would need a different approach
